...
abstract contract BebopSigning {
    ...
    bytes32 private constant MULTI_ORDER_TYPE_HASH =
        0x34728ce057ec73e3b4f0871dced9cc875f5b1aece9fd07891e156fe852a858d9;
    ...
    //                         keccak256(abi.encodePacked(updatedMakerAmounts)),
    //                         order.receiver,
    ...
    //                         keccak256(
    //                             _encodeTightlyPackedNested(order.taker_tokens)
    //                         ),
    //                         keccak256(
    //                             _encodeTightlyPackedNested(order.maker_tokens)
    //                         ),
    //                         keccak256(
    ...
    function _validateSignature(
        ...
        if (signatureType == Signature.Type.EIP712) {
            ...
            if (
                signer != validationAddress &&
                (!isMaker || !orderSignerRegistry[validationAddress][signer])
            ) {
                revert InvalidEIP721Signature();
            }
        } else if (signatureType == Signature.Type.EIP1271) {
            if (
                IERC1271(validationAddress).isValidSignature(hash, signature) !=
                EIP1271_MAGICVALUE
            ) {
    ...
    function _invalidateOrder(address maker, uint256 nonce) private {
        ...
        mapping(uint256 => uint256)
            storage invalidatorStorage = makerNonceValidator[maker];
    ...
    function _validateSingleOrder(
        ...
        (, Signature.Type signatureType) = Signature.extractMakerFlags(
            makerSignature.flags
        );
        _validateSignature(
            order.maker_address,
    ...
    //     _invalidateOrder(order.maker_address, order.maker_nonce);
    //     if (
    //         order.taker_tokens.length != order.taker_amounts.length ||
    //         order.maker_tokens.length != order.maker_amounts.length
    //     ) revert TokensLengthsMismatch();
    //     if (
    //         order.commands.length !=
    //         order.taker_tokens.length + order.maker_tokens.length
    //     ) revert InvalidCommandsLength();
    //     if (order.taker_tokens.length != 1 && order.maker_tokens.length != 1)
    //         revert ManyToManyNotSupported();
    ...
    //     if (
    //         order.maker_addresses.length != makersNum ||
    //         order.maker_nonces.length != makersNum ||
    //         order.taker_tokens.length != makersNum ||
    //         order.maker_tokens.length != makersNum ||
    //         order.taker_amounts.length != makersNum ||
    //         order.maker_amounts.length != makersNum
    //     ) revert OrdersLengthsMismatch();
    //     uint tokenTransfers;
    //     for (uint i; i < makersNum; ++i) {
    //         if (
    //             order.maker_tokens[i].length != order.maker_amounts[i].length ||
    ...
    //                 order.commands,
    //                 tokenTransfers,
    //                 order.maker_tokens[i].length + order.taker_tokens[i].length
    //             ),
    //             0
    //         );
    //         bytes32 multiOrderHash = hashMultiOrder(
    //             partialAggregateOrder,
    //             Order.extractPartnerId(order.flags),
    //             order.maker_amounts[i],
    //             order.maker_nonces[i]
    //         );
    ...
    //             order.maker_addresses[i],
    //             multiOrderHash,
    //             makersSignatures[i].signatureBytes,
    //             signatureType,
    //             true
    //         );
    ...
    //             order.taker_tokens[i].length;
    //     }
    //     if (tokenTransfers != order.commands.length)
    //         revert CommandsLengthsMismatch();
    //     if (order.expiry <= block.timestamp) revert OrderExpired();
    // }
    ...
    function _validateTakerSignatureForSingleOrder(
        ...
        if (order.maker_amount < takerQuoteInfo.makerAmount)
        ...
        if (msg.sender != order.taker_address) {
            Signature.Type signatureType = Order.extractSignatureType(
                order.flags
            );
            _validateSignature(
                order.taker_address,
    ...
    // ) internal {
    //     if (takerQuoteInfo.makerAmounts.length != order.maker_amounts.length)
    ...
    //         if (order.maker_amounts[i] < takerQuoteInfo.makerAmounts[i])
    //             revert UpdatedMakerAmountsTooLow();
    //     }
    //     if (takerQuoteInfo.makerNonce != order.maker_nonce) {
    //         _invalidateOrder(order.maker_address, takerQuoteInfo.makerNonce);
    ...
    //         if (
    //             takerQuoteInfo.makerAmounts[i].length !=
    //             order.maker_amounts[i].length
    //         ) revert MakerAmountsLengthsMismatch();
    //         for (uint j; j < order.maker_amounts[i].length; ++j) {
    //             if (
    //                 order.maker_amounts[i][j] <
    //                 takerQuoteInfo.makerAmounts[i][j]
    //             ) revert UpdatedMakerAmountsTooLow();
    //         }
    //         if (takerQuoteInfo.makerNonces[i] != order.maker_nonces[i]) {
    //             _invalidateOrder(
    //                 order.maker_addresses[i],
    //                 takerQuoteInfo.makerNonces[i]
    //             );
    //         }
    //     }
    //     if (msg.sender != order.taker_address) {
